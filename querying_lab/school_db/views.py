...
def problem_one(request):
    ...
    students = Student.objects.filter(gpa__gte = 3)
...
def problem_three(request):
    ...
    student_courses = StudentCourse.objects.filter(grade='A+').exclude(grade='C+').order_by('student') #and StudentCourse.objects.exclude(grade='C+')
...
def problem_five(request):
    ...
    # Order by last name.
    student_courses =StudentCourse.objects.filter(course__name='Programming', grade='A').order_by('student')
    context = {
        'student_courses': student_courses
    }
